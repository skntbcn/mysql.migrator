...
import time as tm
import signal
from connections import source_config, destination_config
from databases import sys_databases, databases_to_migrate, databases_to_avoid
...
from progress import create_pbar, update_pbar, close_pbar, update_pos_pbar, PbarColors, PbarPrompts
...
def migrate_database_tables(db_name: str, tables: List[str], batch_size: int) -> None:
    ...
    # Iterate over each table and migrate its content
    for table in tables:
        # Migrate the table data
        res, ex = migrate_table_data(src_cur=src_cur, dst_cur=dst_cur, src_conn=src_conn,
                                     dst_conn=dst_conn, db_name=db_name, table_name=table, original_batch_size=batch_size)

        # If migration failed or validation fails, rollback and raise an exception
        if not res or not migration_success(db_name=db_name, table_name=table, src_cur=src_cur, dst_cur=dst_cur):
...
def migrate_table_data(
    ...
    :param ignore_errors: If True, ignores certain SQL errors (e.g., duplicate keys).
    ...
    while offset < row_count:
        ...
        try:
            ...
            if not ignore_errors:
                ...
                insert_query = f"INSERT IGNORE INTO {escape_column_name(table_name)} ({', '.join(column_names)}) VALUES ({values_placeholder})"
            ...
            # 2013 Connection lost while querying
                ...
                try:
                    ...
                    # on_error_insert_single(rows, insert_query, table_name, columns, dst_cur)
...
def migration_process(args: Dict = None) -> List[str]:
    ...
    log_message(f"  - {args.thcount:,} thread workers for databases. This machine has {os.cpu_count():,} cores.", LogType.COMMENT)
    ...
    if len(src_dbs) > 0:
        ...
        progress = create_pbar(len(src_dbs) + 2, leave=True, colour=PbarColors.DATABASE, units='table')
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('-t', '--thread-count', type=int, default=(os.cpu_count() - 1), dest='thcount',
                        help='Optional. Number of threads to use for database migration. Default is to use all CPU cores except one.')
